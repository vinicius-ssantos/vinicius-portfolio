...
describe("motion tokens", () => {
  ...
  it("defines one reduced-motion policy with static final states", () => {
    expect(REDUCED_MOTION_QUERY).toBe("(prefers-reduced-motion: reduce)");
    expect(animationSource).toContain("@media (prefers-reduced-motion: reduce)");
    expect(animationSource).toContain("opacity: 1 !important;");
    expect(animationSource).toContain("animation: none !important;");
    expect(animationSource).toContain("transition: none !important;");
  });
});
