...
Motion in this portfolio explains hierarchy, state, or technical flow. It is not decorative by default.
...
CSS tokens live in `src/app/styles/tokens.css`. JavaScript durations that participate in runtime behavior live in `src/lib/motion.ts` and are guarded by a unit test against CSS drift.
...
Distances, scales, angles, delays, stagger intervals, and easing curves follow the same naming model. Components should not add an arbitrary duration or easing when an equivalent token exists.
...
Use `motion="..."` only to describe intent. Viewport lifecycle, pausing, observer reuse, and cleanup are handled separately by #43.
...
Automated coverage exists in `src/lib/__tests__/motion.test.ts`, `e2e/motion.spec.ts`, and the existing Hero E2E test.
...
6. Avoid a new animation library unless native CSS and the current observer abstraction cannot meet a measured requirement.
