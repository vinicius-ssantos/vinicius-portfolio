...
test.describe("project detail pages", () => {
  ...
  test("Sentinel exposes lifecycle metadata and verifiable evidence", async ({ page }) => {
    ...
    await expect(
      page.getByText("Editorial update: 2026-07-23", { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      evidenceSection.getByRole("link", { name: "Reproducible runbook" }),
    ).toBeVisible();
    await expect(
      evidenceSection.getByRole("link", { name: "Tests and strategy" }),
    ).toBeVisible();
  });
